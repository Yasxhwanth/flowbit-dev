...
async function executeCandlesNode(
  ...
  const request = {
    ...
    exchangeSegment: data.exchangeSegment as
      | "NSE_EQ"
      | "BSE_EQ"
      | "NSE_FNO"
      | "BSE_FNO"
...
async function executeIndicatorsNode(
  ...
  return calculateIndicators(
    candleData,
    indicatorConfigs as Parameters<typeof calculateIndicators>[1],
  );
}
...
async function executeConditionNode(
  ...
  const upstreamValues = Object.values(inputs);
  const indicators = upstreamValues[0] as Record<
    string,
    number | { line: number; signal: number; histogram: number }
  >;
  ...
  try {
    ...
    console.warn(
      `[Condition] Evaluation failed for "${expression}". Attempting AI correction...`,
    );

    try {
      const availableIndicators = Object.keys(indicators);
      const corrected = await attemptAICorrection(
        expression,
        availableIndicators,
      );
      ...
      if (corrected) {
        console.log(
          `[Condition] AI corrected "${expression}" to "${corrected}"`,
        );
...
async function executeOrderNode(
  ...
  console.log(
    `[Workflow] Fetching credentials for user: ${userId}, broker: ${broker}${dryRun ? " (DRY RUN)" : ""}`,
  );
  ...
  try {
    ...
    const finalOutput =
      terminalNodes.length === 1
        ...
        : Object.fromEntries(
            terminalNodes.map((n) => [n.id, context.get(n.id)]),
          );
