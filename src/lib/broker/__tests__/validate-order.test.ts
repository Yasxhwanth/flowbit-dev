...
describe("validateOrder", () => {
  describe("order type validation", () => {
    it("should accept MARKET orders for all brokers", () => {
      expect(() =>
        validateOrder({ ...baseOrder, orderType: "MARKET" }, "dhan"),
      ).not.toThrow();
      expect(() =>
        validateOrder({ ...baseOrder, orderType: "MARKET" }, "fyers"),
      ).not.toThrow();
      expect(() =>
        validateOrder({ ...baseOrder, orderType: "MARKET" }, "angel"),
      ).not.toThrow();
    });

    it("should accept LIMIT orders for all brokers", () => {
      expect(() =>
        validateOrder({ ...baseOrder, orderType: "LIMIT", price: 100 }, "dhan"),
      ).not.toThrow();
      expect(() =>
        validateOrder(
          { ...baseOrder, orderType: "LIMIT", price: 100 },
          "fyers",
        ),
      ).not.toThrow();
      expect(() =>
  ...
  describe("product type validation", () => {
    it("should accept CNC for Dhan", () => {
      expect(() =>
        validateOrder({ ...baseOrder, productType: "CNC" }, "dhan"),
      ).not.toThrow();
    });

    it("should reject DELIVERY for Dhan (Dhan uses CNC)", () => {
      expect(() =>
        validateOrder({ ...baseOrder, productType: "DELIVERY" }, "dhan"),
      ).toThrow(BrokerValidationError);
    });

    it("should accept DELIVERY for Angel", () => {
      expect(() =>
        validateOrder({ ...baseOrder, productType: "DELIVERY" }, "angel"),
      ).not.toThrow();
    });
  });
  ...
  describe("securityId requirement", () => {
    it("should require securityId for Dhan", () => {
      ...
      expect(() => validateOrder(order as OrderRequest, "dhan")).toThrow(
  ...
  describe("LIMIT order validation", () => {
    it("should require price for LIMIT orders", () => {
      expect(() =>
        validateOrder({ ...baseOrder, orderType: "LIMIT" }, "dhan"),
      ).toThrow(BrokerValidationError);
    });
  ...
  describe("quantity validation", () => {
    it("should reject zero quantity", () => {
      expect(() =>
        validateOrder({ ...baseOrder, quantity: 0 }, "dhan"),
      ).toThrow(BrokerValidationError);
    });

    it("should reject negative quantity", () => {
      expect(() =>
        validateOrder({ ...baseOrder, quantity: -1 }, "dhan"),
      ).toThrow(BrokerValidationError);
    });
  });
});
