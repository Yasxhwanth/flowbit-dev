...
export async function marketData(
  ...
  const fromTimestamp = Math.floor(
    (request.fromTimestamp ?? defaultFrom) / 1000,
  );
 ...
 */
function normalizeFyersCandles(
  response: FyersCandleResponse,
): NormalizedCandles {
...
export async function placeOrder(
  ...
  if (response.s !== "ok" || !response.id) {
    throw new BrokerAPIError(
      `Fyers: Order failed - ${response.message}`,
      response.code,
    );
  }
...
function validateOrderRequest(request: OrderRequest): void {
  ...
  if (request.orderType === "LIMIT" && !request.price) {
    throw new BrokerValidationError(
      "Fyers: Price is required for LIMIT orders",
    );
  }
}
