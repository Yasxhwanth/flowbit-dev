...
export function createDhanAdapter(
  ...
  return {
    ...
    placeOrder: (request) =>
      placeDhanOrder(brokerFetch, request, clientId ?? ""),
  };
}
...
function validateOrderRequest(request: OrderRequest, clientId: string): void {
  if (!clientId?.trim()) {
    throw new BrokerValidationError(
      "DHAN_CLIENT_ID environment variable is required",
    );
  }
  ...
  if (
    ...
    !(request.exchangeSegment in EXCHANGE_SEGMENT_MAP)
  ) {
    throw new BrokerValidationError(
      `Invalid exchange segment. Must be one of: ${Object.keys(EXCHANGE_SEGMENT_MAP).join(", ")}`,
    );
 ...
 */
function normalizeCandleResponse(
  response: DhanCandleResponse,
): NormalizedCandles {
