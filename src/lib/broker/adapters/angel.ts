...
export async function marketData(
  ...
  const fromDate = request.fromTimestamp
    ? new Date(request.fromTimestamp)
    : defaultFrom;
  ...
  try {
    const response = await brokerFetch<AngelCandleResponse>(
    ...
    throw new BrokerAPIError(
      `Angel: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
 ...
 */
function normalizeAngelCandles(
  response: AngelCandleResponse,
): NormalizedCandles {
...
export async function placeOrder(
  ...
  try {
    const response = await brokerFetch<AngelOrderResponse>(
...
function validateOrderRequest(request: OrderRequest): void {
  ...
  if (request.orderType === "LIMIT" && !request.price) {
    throw new BrokerValidationError(
      "Angel: Price is required for LIMIT orders",
    );
  }
}
