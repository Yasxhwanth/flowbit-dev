...
export async function runBacktest(
  ...
  console.log(
    `[Backtest] Period: ${new Date(from).toISOString()} to ${new Date(to).toISOString()}`,
  );
  ...
  console.log(
    `[Backtest] Running ${candles.close.length} candles with lookback ${lookbackPeriod}`,
  );
  ...
  for (let i = lookbackPeriod; i < candles.close.length; i++) {
    ...
    try {
      ...
      const indicatorConfigs =
        (indicatorsNode?.data?.indicators as Parameters<
      ...
      if (conditionMet) {
        ...
        if (canExecute) {
          ...
          console.log(
            `[Backtest] ${orderType} at ${currentPrice} on ${new Date(timestamp).toISOString()}`,
          );
        }
      }
    } catch (error) {
  ...
  console.log(
    `[Backtest] Completed: ${trades.length} trades, PNL: ${metrics.totalPNL.toFixed(2)}`,
  );
