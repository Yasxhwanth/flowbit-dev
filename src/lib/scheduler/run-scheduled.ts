 ...
 */
function convertToWorkflowGraph(workflow: {
  userId: string;
      ...
      try {
        ...
        if (!graph) {
          ...
          results.push({
            ...
            logs: [],
            error:
              "Workflow uses unsupported node types for scheduled execution",
            durationMs: performance.now() - workflowStartTime,
          });
          continue;
        }
        ...
        console.log(
          `[Scheduler] Executing workflow: ${workflow.id}, userId: ${workflow.userId}, name: ${workflow.name}`,
        );
