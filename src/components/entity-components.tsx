import {  AlertTriangleIcon, Loader2Icon, MoreHorizontalIcon, MoreVerticalIcon, PackageOpenIcon, Plus, PlusIcon, SearchIcon, TrashIcon } from "lucide-react";
    ...
    return (
        <div className="flex flex-row items-center justify-between gap-x-4">
          <div className="flex flex-col">
            ...
            {description && (
              <p className="text-vs md:text-sm text-muted-foreground">
                {description}
              </p>
            )}
          </div>
...
export function EntityList<T>({
  ...
  if (!items || items.length === 0 && emptyView) {
    return (
  ...
  export const EntityItem = ({
    ...
    const handleRemove = async (e: React.MouseEvent | Event) => {
      e.preventDefault();
      e.stopPropagation();
  ...
  {(actions || onRemove) && (
  <CardFooter className="flex gap-x-4 items-center">
    {actions}
    {onRemove && (
      <DropdownMenu>
  ...
  </CardFooter>  // <-- FIX: now inside the condition
)}
