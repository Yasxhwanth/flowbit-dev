  ...
  try {
    ...
    for (const exec of executions) {
      ...
      for (const orderLog of orderLogs) {
        const orderOutput = orderLog.output as
          | Record<string, unknown>
          | undefined;
        if (orderOutput?.orderId) {
          // Extract candles node for symbol/broker
          const candlesNode = exec.workflow.nodes.find(
            (n) => (n.data as Record<string, unknown>)?.symbol,
          );
          const candlesData = candlesNode?.data as
            | Record<string, unknown>
            | undefined;
