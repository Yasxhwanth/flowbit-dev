...
export const slackExecutor: NodeExecutor<SlackNodeData> = async ({
  ...
  if (!data.content) {
    ...
    throw new NonRetriableError(
      `Message content is required. Data received: ${JSON.stringify(data)}`,
    );
  }
  ...
  try {
    ...
    if (!data.content) {
      await publish(slackChannel().status({ nodeId, status: "error" }));
      throw new NonRetriableError(
        `Content is required but was ${data.content === null ? "null" : "undefined"}`,
      );
    }
    ...
    if (typeof data.content !== "string") {
      await publish(slackChannel().status({ nodeId, status: "error" }));
      throw new NonRetriableError(
        `Invalid content type: expected string, got ${typeof data.content}. Value: ${JSON.stringify(data.content)}`,
      );
    }
    ...
    if (!messageText || messageText === "null" || messageText === "undefined") {
      await publish(slackChannel().status({ nodeId, status: "error" }));
      throw new NonRetriableError(
        `Resolved message content is invalid. Original: "${data.content}", Resolved: "${resolvedContent}", Context keys: ${Object.keys(context).join(", ")}`,
      );
    }
    ...
    const result = await step.run("slack-webhook", async () => {
      ...
      console.log(
        "[Slack Executor] Sending payload:",
        JSON.stringify(payload, null, 2),
      );
